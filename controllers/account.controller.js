...
const signUpController = errHandel(async (req, res , next) => {
    
        
    const createNewUser = await createUser(req.body)
    if (createNewUser) {
        let token = jwt.sign({ id: req.body.id }, process.env.JWT_KEY);
...
const loginController = errHandel (function (req, res) {
    bcrypt.compare(req.body.password, req.user.password, function (err, result) {
        ...
        if (result) {
            let accessToken = jwt.sign({ id: req.user.id }, process.env.JWT_KEY, { expiresIn: '1d' })
...
const reqestRefreshToken = errHandel( async (req, res) => {
    ...
    const decodeAccount = jwt.verify(refreshToken, process.env.JWT_KEY)
        let user = await User.findByPk(decodeAccount.id)
        if (user) {
            refreshTokens = refreshTokens.filter((token) => token !== refreshToken)
            let newRefreshToken = jwt.sign({ id: user.id }, process.env.JWT_KEY, { expiresIn: '1d' })
